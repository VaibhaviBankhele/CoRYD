...
export default function IncomingRideRequest({ ride, onAccept, onReject, loading }) {
  const calculateDistance = () => {
    if (!ride || !ride.pickupLocation || !ride.dropLocation) return 0;
  ...
  const estimatedFare = (50 + parseFloat(distance) * 10).toFixed(0);
      ...
      <div className="bg-white rounded-lg shadow-2xl max-w-md w-full p-6 animate-bounce">
        ...
        <div className="space-y-3 mb-6">
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm font-semibold text-gray-800">{ride?.pickupLocation?.name}</p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm font-semibold text-gray-800">{ride?.dropLocation?.name}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6 bg-gray-50 p-4 rounded-lg">
          ...
          <div className="text-center">
            ...
            <p className="text-lg font-bold text-blue-600">{ride?.availableSeats}</p>
          </div>
        </div>
